...
const WindowList = GObject.registerClass({GTypeName: 'WindowList'}, class WindowList extends GObject.Object {
    _init() {
        this.apps_menu = new St.BoxLayout({});
        this.actor = this.apps_menu;
        this._updateMenu();
        this._restacked = global.display.connect('restacked', this._updateMenu.bind(this));
        this._window_change_monitor = global.display.connect('window-left-monitor', this._updateMenu.bind(this));
        this._workspace_removed = global.workspace_manager.connect('workspace-removed', this._updateMenu.bind(this));
        this._workspace_changed = global.workspace_manager.connect('active-workspace-changed', this._updateMenu.bind(this));
        this._workspace_added = global.workspace_manager.connect('workspace-added', this._updateMenu.bind(this));
    }
    ...
    _activateWindow(ws, w) {
        const is_active_ws = global.workspace_manager.get_active_workspace() === ws;
